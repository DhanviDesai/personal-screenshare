...
def test_retry_after_failed_share_attempt_without_holding_lock(client):
    user = client.post("/personal/screenshare/api/sessions/perm-room/token", json={"displayName": "User"}).json()
    ...
    start = client.post(
        "/personal/screenshare/api/sessions/perm-room/screen-share/start",
