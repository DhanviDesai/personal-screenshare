...
def test_screen_share_lock_unaffected_by_other_participants(client):
    a = client.post("/api/sessions/cam-room/token", json={"displayName": "A"}).json()
    b = client.post("/api/sessions/cam-room/token", json={"displayName": "B"}).json()

    start = client.post(
        "/api/sessions/cam-room/screen-share/start",
    ...
    leave_b = client.post(
        "/api/sessions/cam-room/leave",
        headers={"Authorization": f"Bearer {b['token']}"},
    )
    assert leave_b.status_code == 200
    assert leave_b.json()["lockReleased"] is False

    still = client.get("/api/sessions/cam-room/presenter").json()
