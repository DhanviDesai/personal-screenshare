...
def test_join_and_share_lock_flow(client):
    presenter = client.post(
        "/personal/screenshare/api/sessions/demo-session/token", json={"displayName": "Presenter"}
    ).json()
    viewer = client.post(
        "/personal/screenshare/api/sessions/demo-session/token", json={"displayName": "Viewer"}
    ).json()

    start = client.post(
        "/personal/screenshare/api/sessions/demo-session/screen-share/start",
        headers={"Authorization": f"Bearer {presenter['token']}"},
    )
    assert start.status_code == 200

    state = client.get("/personal/screenshare/api/sessions/demo-session/presenter")
    ...
    stop = client.post(
        "/personal/screenshare/api/sessions/demo-session/screen-share/stop",
        headers={"Authorization": f"Bearer {presenter['token']}"},
    )
    assert stop.status_code == 200

    after = client.get("/personal/screenshare/api/sessions/demo-session/presenter").json()
